                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-2 flex gap-2">
                                        <a href="{{ route('users.show', $user->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded bg-blue-100 hover:bg-blue-200 text-blue-600" title="Detail">
                                            <i class="fa-solid fa-eye"></i>
                                        </a>
                                        <a href="{{ route('users.edit', $user->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded bg-yellow-100 hover:bg-yellow-200 text-yellow-600" title="Edit">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <form id="delete-form-{{$user->id}}" action="{{ route('users.destroy', $user->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button onclick="confirmDelete('{{$user->id}}')" type="button" class="inline-flex items-center justify-center w-8 h-8 rounded bg-red-100 hover:bg-red-200 text-red-600" title="Delete">
                                ...
                                @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-2 text-center">Tidak ada data pengguna yang ditemukan.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
