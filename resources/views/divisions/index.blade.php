                        ...
                        <table id="items-table" class="table-auto w-full text-left text-sm text-gray-500 dark:text-gray-400 mt-4">
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    <td class="px-4 py-2">{{ $division->id }}</td>
                                    <td class="px-4 py-2">{{ $division->name }}</td>
                                    <td class="px-4 py-2">{{ $division->description }}</td>
                                    <td class="px-4 py-2 flex gap-2">
                                        <a href="{{ route('divisi.show', $division->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded bg-blue-100 hover:bg-blue-200 text-blue-600" title="Detail">
                                            <i class="fa-solid fa-eye"></i>
                                        </a>
                                        <a href="{{ route('divisi.edit', $division->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded bg-yellow-100 hover:bg-yellow-200 text-yellow-600" title="Edit">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </a>
                                        <form id="delete-form-{{$division->id}}" action="{{ route('divisi.destroy', $division->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button onclick="confirmDelete('{{$division->id}}')" type="button" class="inline-flex items-center justify-center w-8 h-8 rounded bg-red-100 hover:bg-red-200 text-red-600" title="Delete">
                                ...
                                @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-2 text-center">Tidak ada data divisi yang ditemukan.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                        <div class="mt-4">
                            {{ $divisions->withQueryString()->links('pagination::tailwind') }}
                        </div>
                    </div>
                </div>
            </div>
