        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    {{-- Form Edit Data --}}
                    <form id="formsub" method="POST" action="{{ route('sarana.update', $item->id) }}" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="number" name="purchase_year" id="purchase_year" value="{{ old('purchase_year', $item->purchase_year) }}" required min="1900" max="{{ date('Y') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                        ...
                        <div>
                            ...
                            @if($item->photo_path)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/' . $item->photo_path) }}" alt="Foto Sarana" class="h-24 rounded">
                                </div>
                            @endif
                            @error('photo_path')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
